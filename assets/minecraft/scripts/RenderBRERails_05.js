...
function shouldRenderObject(tileEntity, objName, len, pos) {
    ...
    if (renderer.isSwitchRail(tileEntity)) {
        ...
        if (fixtureL.containsName(objName) || fixtureR.containsName(objName) || leftParts.containsName(objName) || rightParts.containsName(objName)) {
            return true;
        }
        return false;
    }
    ...
    if (renderer.isSwitchRail(tileEntity)) {
        ...
        for (var i = 0; i <= (segmentMax - 1); ++i) {
            ...
            for (var j = 0; j <= sleepermax0; ++j) {
                ...
                var y2 = (rm2.getRailHeight(sleepermax1, (sleeperOfset + j)) - origHeight + origCantHeigt);
