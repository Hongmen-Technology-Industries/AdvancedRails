...
function getCurrentRailIndex() {
    if (currentRailIndexField === undefined) {
        currentRailIndexField = (isLegacy() ? RailPartsRenderer.class : RailPartsRendererBase.class).getDeclaredField("currentRailIndex");
...
function getBaseRailName(tileEntity) {
    return this.isLegacy() ? tileEntity.getProperty().railModel : tileEntity.getResourceState().getResourceName();
}
